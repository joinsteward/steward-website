...
import { uploadNewFile } from 'utils/fileUpload'
...
const FileUpload = ({
  ...
  const handleFileChange = async (file) => {
    ...
    const upload = await uploadNewFile(file, setProgress, setUploading, getResult)
    ...
    return (
      <div className="relative h-full">
        ...
        >
          ...
          />
          <div className={classNames('flex flex-col ', { 'h-full': fullHeight })}>
            {(group || label) && (
              <span className="inline-flex  w-7/12 max-w-3xl items-center border-gray-300 py-1 text-sm text-gray-800">
                {group || label}
              </span>
            )}
            {uploadedDiv && preview && (
              ...
              <>
                {!uploadedDiv && (
                  <label
                    ...
                    htmlFor={name}
                    className={classNames(
                      'flex h-full w-full cursor-pointer items-center rounded-r-lg  border border-gray-200 bg-white px-4 py-3.5 text-sm text-gray-500 transition-colors ease-in-out hover:border-gray-900 active:border-gray-900 active:outline-none active:ring-4 active:ring-gray-400 active:ring-opacity-30',
                      dragActive ? 'bg-white' : '',
                      { ['disabled']: disabled }
                    )}
                  >
